use web_sys::{wasm_bindgen::JsCast, Document, DocumentFragment, Element};
...
pub fn has_any_concrete_roles(element: &Element, roles: Vec<&str>) -> bool {
    get_role(element).is_some_and(|role| roles.contains(&role.as_str()))
}
